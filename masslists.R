...
fragment_intensities <- function(data = ass_data_merged, subset = FALSE, uvhcpd = FALSE, Plot_title_suffix = "", Plot_dir_name = "plot/", Plot_file_name = "pin1_6al7_2phos_sty", Plot_width = 3, Plot_height = 10) {

  Data_dir <- getwd()

  data <- ass_data_merged[!(ass_data_merged$m.z > 2200 & ass_data_merged$Charge == 1) & !(ass_data_merged$m.z > 4800 & ass_data_merged$Charge == 2),]

  #levels(ass_data_merged$hcd) <- c("20","40","60","80","100","120","140","160","180")
  #ass_data_merged <- ass_data_merged[!duplicated(ass_data_merged[6:8]),]
  data$energy <- as.numeric(data$energy)

  if (subset == TRUE) data <- subset(data, abs(data$calib.ppm) <= 2.0)

  #ass_data_merged$hcd <- factor(ass_data_merged$hcd, levels = c("160","140","120","100","80","60","40","20"))
  #ass_data_merged$energy <- factor(ass_data_merged$energy, levels = c("2","1.5","1","0.5"))
  #ass_data_merged$fragment <- paste0(ass_data_merged$Sequence.position," ",ass_data_merged$Ion.type," ",ass_data_merged$Series.number, " ", ass_data_merged$Mass.shift)
  #ass_data_merged$fragment <- paste0(ass_data_merged$Sequence.position, " ", ass_data_merged$exp)
  #ass_data_dup <- ass_data_merged[duplicated(ass_data_merged[,c("Sequence.position", "Ion.type", "Series.number", "Mass.shift")]) | duplicated(ass_data_merged[,c("Sequence.position", "Ion.type", "Series.number", "Mass.shift")], fromLast = TRUE),]
  #ass_data_dup <- ass_data_merged[duplicated(ass_data_merged[,c("Sequence.position","exp")]) | duplicated(ass_data_merged[,c("Sequence.position", "exp")], fromLast = TRUE),]
  #df <- ass_data_merged[!(ass_data_merged$fragment %in% ass_data_dup$fragment),]
  #df <- count(unique(ass_data_merged[,c("exp.name","Sequence.position")]), vars = "exp.name")
  #df$seq.cov <- (df$freq/(nchar(seq)-1))*100
  #df["all","seq.cov"] <- (length(unique(ass_data_merged$Sequence.position))/(nchar(seq)-1))*100
  ...
  plot_gg <- ggplot(data) +
    stat_identity(data = data, geom = "tile", width  = 1, aes(x = Sequence.position, y = mod, fill = norm.intens.Zscored)) +
    ...
    #geom_vline(xintercept = sn, color = "red", size = 1) +
    #geom_histogram(aes(color = Ion.type)) +
    #geom_linerange (aes(x = Sequence.position, y = norm.intens), position = position_dodge()) +
    #stat_identity(aes(color = Ion.type)) +
    #scale_y_discrete( expand = c(0,0), limits= c(1,1)) +
    ...
    ylab("Number of Phosphorylations") + # x label
    xlab("Sequence position") +  # y label
    #ggtitle(paste0("Sequence Coverage\n", Plot_title_suffix)) +
    ...
    geom_text(data = sty.df, aes(y = 3.6, x = Sequence.position, label = Amino.acid), size = 2) +
